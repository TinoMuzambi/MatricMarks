...
math.pass.type <- maths.long %>% 
  ggplot(aes(x = matric_class)) +
  geom_line(aes(y =  thirty, group = province)) +
  geom_line(aes(y = fourty, group = province), linetype = "dashed") +
  geom_line(aes(y = fifty, group = province), color = "red") +
  facet_wrap(~province, scales = "free_y") +
  labs(x = "Matric Class", y = "Percentage of Learners", color = "Type") +
  theme_minimal() +
  theme(
    panel.background = element_rect(fill='transparent'), #transparent panel bg
    plot.background = element_rect(fill='transparent', color=NA), #transparent plot bg
